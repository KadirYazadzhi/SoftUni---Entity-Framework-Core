...
public class ChangeTracker<TEntity> where TEntity : class, new() {
    ...
    public IEnumerable<TEntity> GetModifiedEntities(DbSet<TEntity> dbSet) {
        ...
        foreach (TEntity proxyEntity in this.AllEntities) {
            ...
            if (isModified) modifiedEntities.Add(localEntity);
    ...
    private static IEnumerable<object> GetPrimaryKeyValues(TEntity entity, PropertyInfo[] primaryKeys) => primaryKeys.Select(pk => pk.GetValue(entity))!;
